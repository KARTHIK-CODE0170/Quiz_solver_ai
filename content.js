...
(function () {
  ...
  const isAdityaPlatform = window.location.hostname.includes('maya.adityauniversity.in');
  const isAssignmentsPage = /\/home\/(assignments|grades)/.test(href);
  const isQuizPage = isAdityaPlatform ? /\/grand-assessment\//.test(href) : /\/(assignment-submission|quiz|exam)(\/|\b)/.test(href);
  ...
  async function startPipeline(targetPhase = 'quiz') {
    if (!isAdityaPlatform && !courseSlug) {
      setStatus('⚠ Not on a supported course page');
      return;
    }
  ...
  function findBlocks() {
    ...
    if (isAdityaPlatform) {
      document.querySelectorAll('.card-body').forEach(n => {
        if (n.querySelector('label.form-label')) {
          results.push(n);
        }
      });
      return results;
  ...
  function getQuestion(b) {
    if (isAdityaPlatform) {
      const label = b.querySelector('label.form-label');
      if (label) {
        return (label.innerText || '').replace(/\s+/g, ' ').trim().slice(0, 1200);
      }
    }
  ...
  function getOptions(b) {
    ...
    if (isAdityaPlatform) {
      const opts = b.querySelectorAll('.form-check-label span');
      return [...opts].map(o => (o.innerText || '').replace(/\s+/g, ' ').trim()).filter(Boolean);
    }
